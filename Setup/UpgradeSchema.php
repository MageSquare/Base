...
namespace MageSquare\Base\Setup;
...
class UpgradeSchema implements UpgradeSchemaInterface
{
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(), '1.0.1', '<')) {
            $this->createMageSquareBaseModuleTable($setup);
        }

        if (version_compare($context->getVersion(), '1.1.0', '<')) {
            $this->addComposerColumnsToMageSquareBaseModuleTable($setup);
        }
    ...
    protected function createMageSquareBaseModuleTable(SchemaSetupInterface $setup)
    {
        $table = $setup->getConnection()
            ->newTable($setup->getTable('magesquare_base_module'))
            ->addColumn(
                'code',
                Table::TYPE_TEXT,
                50,
    ...
    protected function addComposerColumnsToMageSquareBaseModuleTable(SchemaSetupInterface $setup)
    {
        $table = $setup->getTable('magesquare_base_module');
