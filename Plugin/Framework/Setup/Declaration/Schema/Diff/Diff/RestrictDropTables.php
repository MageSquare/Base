...
namespace MageSquare\Base\Plugin\Framework\Setup\Declaration\Schema\Diff\Diff;
...
{
    /**
     * Restrict to delete MageSquare tables throw Declarative Schema.
     *
     * @param Diff $subject
     * @param bool $result
    ...
    public function afterCanBeRegistered(Diff $subject, bool $result, ElementInterface $object, $operation): bool
    {
        if ($result === true
            ...
            && stripos($object->getName(), 'magesquare') !== false
        ) {
            $result = false;
        }
