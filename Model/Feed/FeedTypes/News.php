...
namespace MageSquare\Base\Model\Feed\FeedTypes;

use MageSquare\Base\Model\AdminNotification\Model\ResourceModel\Inbox\Collection\ExistsFactory;
use MageSquare\Base\Model\Config;
use MageSquare\Base\Model\Feed\FeedContentProvider;
use MageSquare\Base\Model\ModuleInfoProvider;
use MageSquare\Base\Model\Parser;
...
class News
{
    ...
    protected function isItemValid(\SimpleXMLElement $item): bool
    {
        return $this->validateByExtension((string)$item->extension)
            && $this->validateByMageSquareCount($item->magesquare_module_qty)
            && $this->validateByNotInstalled((string)$item->magesquare_module_not)
     ...
     * @return string[]
     */
    private function getInstalledMageSquareExtensions(): array
    {
        if (!$this->magesquareModules) {
            $modules = $this->moduleList->getNames();
            ...
            $modules = array_filter(
                ...
                static function ($item) {
                    return strpos($item, 'MageSquare_') !== false;
                }
            );
            $this->magesquareModules = $modules;
        }

        return $this->magesquareModules;
    }
    ...
    private function validateByExtension(string $extensions, bool $allModules = false): bool
    {
        if ($extensions) {
            ...
            if ($arrExtensions) {
                $installedModules = $allModules ? $this->getAllExtensions() : $this->getInstalledMageSquareExtensions();
    ...
    private function validateByNotInstalled(string $extensions): bool
    {
        if ($extensions) {
            ...
            if ($arrExtensions) {
                $installedModules = $this->getInstalledMageSquareExtensions();
     ...
     * @return bool
     */
    private function validateByMageSquareCount($counts): bool
    {
        $result = true;
        ...
        if ($countString) {
            ...
            $magesquareModules = $this->getInstalledMageSquareExtensions();
            $dependModules = $this->getDependModules($magesquareModules);
            $magesquareModules = array_diff($magesquareModules, $dependModules);

            $magesquareCount = count($magesquareModules);

            if ($magesquareCount
                && (
                    in_array($magesquareCount, $arrCounts, false) // non strict
                    || ($moreThan && $magesquareCount >= $moreThan)
                )
            ) {
                $result = true;
            }
    ...
    /**
     * @param string[] $magesquareModules
     *
     * @return array
     */
    private function getDependModules(array $magesquareModules): array
    {
        $depend = [];
        $result = [];
        $dataName = [];
        foreach ($magesquareModules as $module) {
            ...
            if (isset($data['require']) && is_array($data['require'])) {
                foreach ($data['require'] as $requireItem => $version) {
                    if (strpos($requireItem, 'magesquare') !== false) {
                        $depend[] = $requireItem;
                    }
                }
            }
